...
import { processingIds } from '../crons/clearTempFiles.js';
...
export const renderERDfromSchema = async (schema, outputFormat = 'svg', theme = 'default') => {
  await fs.mkdir(config.tmpFolder, { recursive: true });

  const id = uuid();
  const mmdFile = path.join(config.tmpFolder, `${id}.mmd`);
  /**
   * @type {OutFile}
   */
  const outFile = /** * @type {OutFile} */ (path.join(config.tmpFolder, `${id}.out`));
